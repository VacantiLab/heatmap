...
#     One instance of thses cases is in KICH for gene TRIM60
{
    ...
    if ("try-error" %in% class(attempt))
    {
        CoxResultsDataFrame <- data.frame(matrix(nrow=2,ncol=8))
        rownames(CoxResultsDataFrame) <- c(gene,'Age')
        colnames(CoxResultsDataFrame) <- c('CoxParameters','HazardModelCoefficients','HazardModelCoefficientLowerBounds','HazardModelCoefficientUpperBounds','Identifier','CoxLikelihoodRatioWaldP','CoxGeneP','Risk')
        CoxResultsDataFrame$CoxParameters <- c(gene,'Age')
        CoxResultsDataFrame$HazardModelCoefficients <- c(0,0)
        CoxResultsDataFrame$HazardModelCoefficientLowerBounds <- c(0,0)
        CoxResultsDataFrame$HazardModelCoefficientUpperBounds <- c(0,0)
        CoxResultsDataFrame$Identifier <- c(DataFrameIdentifier,DataFrameIdentifier)
