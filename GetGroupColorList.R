GetGroupColorList <- function(GROUP_KEY,DATA,group_color_designations_file,ColGroupsScheme,sep_char)
{
  COLOR_KEY <- read.table(file=group_color_designations_file,head=TRUE,sep=sep_char,stringsAsFactors=FALSE,comment.char="",check.names=FALSE) #reads the colors associated with the groups, check.names=FALSE ensures text in column names is not changed
